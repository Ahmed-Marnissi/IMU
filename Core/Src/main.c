...
/* USER CODE BEGIN PV */
 ...
 uint8_t MagI2cAddress =AK8963_ADDRESS;
   ...
   float MagneticDeclination = 3.1;  //bizerte en deg
 ...
 int IsConnectedMPU9250() {
...

   {
    ...
    void Update()
    {
        ...
        float YawMag(float Buffer[])
        {
          static const float MagneticDeclination = 0.03490659 ;
          //magnetic declination in tunisia (Ariana) in rad :
          SetMagneticDeclination(   0.03490659);

          //initial yaw  :
          static float Yaw = 0.0;
          static float YawDegrees = 0.0 ;

          Yaw = atan2f(Buffer[1],Buffer[0]) + MagneticDeclination ;
          ...
          YawDegrees = Yaw * 180.0/M_PI; // The heading in Degrees unit
